...
function Delivery() {
  return (
    <div className=' bg-rose-100 py-4 grid grid-cols-1 sm:grid-cols-2 gap-3 '>
        {details?.map((b, i) => (
            <div className='flex flex-col space-y-3 shadow-lg items-center justify-center ' key={i}>
                <h2 className='text-green-500'>{b.icon}</h2>
                <h2 className='text-lg sm:text-xl font-extrabold'>{b.name}</h2>
                <span className='text-sm sm:text-xl'>{b.desc}</span>
