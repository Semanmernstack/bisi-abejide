...
function Delivery() {
  return (
    <div className=' bg-rose-100 dark:bg-gray-800  py-4 grid grid-cols-1 sm:grid-cols-2 gap-3 '>
        {details?.map((b, i) => (
            <div className='flex flex-col  shadow-lg items-center justify-center ' key={i}>
                <h2 className='text-green-500'>{b.icon}</h2>
                <h2 className='text-lg sm:text-xl dark:text-white font-bold'>{b.name}</h2>
                <span className='text-sm dark:text-white md:text-lg italic'>{b.desc}</span>
