...
const getProd = async() => {
    ...
    const query = `*[_type == 'ankara'][0...16]  | order(_createdAt desc) {
        _id,
          price,
          name,
          "slug": slug.current,
...
async function Anka() {
  ...
  return (
    ...
    <h1 className=' items-center text-center bg-[#7CFC00] py-2 px-2 font-serif font-extrabold justify-center p-2 text-2xl md:text-3xl dark:text-white text-gray-700'>ANKARA/ADIRE</h1>
    <div className='grid grid-cols-1 p-2    gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid'>
        ...
        <div className='flex space-y-2  shadow-xl hover:bg-[#0FFF50] items-center justify-center hover:ease-out  rounded-lg p-3  flex-col  transition ease-in-out delay-150  hover:-translate-y-1 hover:scale-95  duration-300   ' key={p._id}>
            ...
            <Link  href={`/anka/${p.slug}`} className="text-center items-center bg-green-600 px-2 py-2 rounded-xl  cursor-pointer p-2 justify-center"    > 
