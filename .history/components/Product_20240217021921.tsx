...
async function Product() {
  ...
  return (
    <div className='flex flex-col transition bg-white  ease-in-out delay-150 hover:-translate-y-1  duration-300  '>
        <h1 className=' items-center text-center font-serif font-extrabold justify-center p-4 text-2xl md:text-3xl dark:text-white text-gray-700'> Products</h1>
        <div className='grid grid-cols-1 p-4     gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid'>
            
        {datas.map(( p: any ) => (
        <div className='flex space-y-2 border-shadow  items-center justify-center hover:ease-out  hover:bg-purple-200 rounded-lg p-3  flex-col  transition ease-in-out delay-150  hover:-translate-y-1 hover:scale-95  duration-300   ' key={p._id}>
          ...
          <Link className="text-center items-center bg-slate-500 px-2 py-2 rounded-xl text-white   cursor-pointer p-2 justify-center"    href={`/product/${p.slug}`}> 
