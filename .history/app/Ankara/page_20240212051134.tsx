...
async function getProductt() {
    const query = `*[_type == 'ankara'][0...2]  | order(_createdAt desc) {
        _id,
          price,
          name,
          "slug": slug.current,
  ...
  return (
    <div className='flex flex-col transition ease-in-out delay-150 hover:-translate-y-1  duration-300  '>
        ...
        <div className='flex space-y-2 border-4  items-center justify-center hover:ease-out  hover:bg-purple-200 rounded-lg p-3  flex-col  transition ease-in-out delay-150  hover:-translate-y-1 hover:scale-95  duration-300   ' key={p._id}>
          ...
          <Link className="text-center items-center  cursor-pointer p-2 justify-center"    href={`/ankara/${p.slug}`}> 
        ...
        <Link className='flex p-4 items-center hover:text-xl cursor-pointer dark:text-white text-violet-600 gap-2' href={'/ankara'}>
